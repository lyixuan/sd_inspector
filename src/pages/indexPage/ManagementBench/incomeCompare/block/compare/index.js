...
class FamilyScoreLeft extends React.Component {
        ...
        render: (groupScore, data) => {
          ...
          return data.isShowPro && PkName ? (
            <Progress
              ...
              data={data}
              PkName={PkName}
              maxNumMyScore={maxNumMyScore}
            />
          ) : (
            <div
              className={styles.pkRankMain}
              style={{ justifyContent: 'flex-satrt', marginRight: '-18px' }}
            >
              <div
                style={{
                  color: '#52C9C2',
  ...
  render() {
    ...
    // const dataSource =
    //   familyScoreList &&
    //   familyScoreList.dimensionList.length > 0 &&
    //   this.fillDataSource(familyScoreList.dimensionList);
    return (
      <div className={styles.compareCon}>
        <div>
          <span className={styles.title}>创收学院对比</span>
        </div>
