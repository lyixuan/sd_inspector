...
import BITextCell from '../BITextCell';
...
function contrastAll({ nums = [], isReversed, colors = colorsArr, ...props }) {
  const orderNums = orderFn([...nums], isReversed);
  return nums.map((item, index) => <BITextCell key={index} bgcolor={item ? getColor(colors, orderNums.indexOf(item)) : ''} {...props}>{item}</BITextCell>
  )
}

class BIContrastCell extends React.Component {
  colorContrastSingle = ({ nums = [], text = 0, isReversed, colors = colorsArr, textContent, ...props }) => {
    const orderNums = orderFn([...nums], isReversed);
    return <BITextCell bgcolor={text ? getColor(colors, orderNums.indexOf(text)) : ''} {...props}>{textContent ? textContent : text}</BITextCell>
  }
  render() {
