...
import styles from './AppealInfo/style.less';
      ...
      <div key={i} className={styles.infoCls} style={{ padding: '20px' }}>
        <Row>
          <Col span={12}>
            {type === 'startAppeal' ? (
              <>
                ...
                {data.attUrl ? (
                  <DownLoad
                    loadUrl={`${STATIC_HOST}/${data.attUrl}`}
                    text={name}
                    fileName={() => name}
              ...
              <>
                <span className={data.checkResult !== 1 ? styles.redIcon : styles.greenIcon}>
                  {' '}
                  审核结果：
                </span>
                <span>
                  {BiFilter('APPEAL_RESULT_TYPE').map(item => {
                    if (item.id === data.checkResult) {
                      return item.name;
                    } else return null;
                  })}
                </span>
          ...
          <Col span={8}>
            <span>操作时间：</span>
            <span>
              {data.operateDate ? moment(data.operateDate).format('YYYY年MM月DD日 HH:mm:ss') : null}
            </span>
          </Col>
        </Row>
        <Row>
