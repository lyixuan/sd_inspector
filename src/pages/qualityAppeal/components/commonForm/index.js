...
class CreateQualityNewSheet extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      JSON.stringify(nextProps.orgMapByMailData) !== JSON.stringify(this.props.orgMapByMailData)
    ) {
      // 使用邮箱获取组织机构信息
  ...
  noCheckoutAction = () => {
    // 审核不通过时,不需要调取检索参数接口
    this.setState({
      isShowOrderNumConfirmModel: true,
      msgDetail: '确认驳回该条记录吗?',
      buttonText: '确定',
    });
  };
        ...
        <BIModal
          ...
          footer={[
            <BIButton
              ...
              onClick={this.changeOrderNumConfirmModel.bind(null, false)}
            >
              取消
            </BIButton>,
            <BIButton
              key="2"
              type="primary"
              onClick={this.confirmModelSubmit}
