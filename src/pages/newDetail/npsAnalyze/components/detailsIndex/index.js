  ...
  columns = () => {
    const columns = [
      ...
      },
      {
        ...
        <Tooltip title={text}><Link to={`/inspector/ko/behaviorPath?params=${encodeURIComponent(JSON.stringify({ userId: record.stuId, target: 'draw' }))}`} target='_black'>{text}</Link></Tooltip>
