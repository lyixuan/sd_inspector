...
class Survey extends React.Component {
  ...
  newData = () => {
    ...
    let newOrgList = JSON.parse(JSON.stringify(orgList)),
      newProvinceJson = JSON.parse(JSON.stringify(provinceJson));
    if (orgList) {
      newOrgList.unshift({ name: '全部学院', id: null, sub: [] });
      newOrgList.map(item => item.sub.unshift({ name: '全部家族', id: null, sub: [] }));
      newOrgList.map(item =>
        item.sub.map(item1 => item1.sub.unshift({ name: '全部小组', id: null, sub: [] }))
      );
    }
    if (provinceJson) {
    ...
    return (
      <>
        <div className={styles.form1}>
          <div>
            <span className={styles.labelCls}>报考省份</span>：
            <BISelect
              options={newProvinceJson}
          ...
          <div>
            <span className={styles.labelCls}>学院</span>：
            <BISelect
              options={newOrgList}
          ...
          <div>
            <span className={styles.labelCls}>家族</span>：
            <BISelect
              options={familyData}
        ...
        <div className={styles.form1}>
          <div>
            <span className={styles.labelCls}>小组</span>：
            <BISelect
              options={groupData}
          ...
          <div>
            <span className={styles.labelCls}>日期</span>：
            <BIRangePicker
          ...
          <div style={{ width: '302px', textAlign: 'right' }}>
            ...
            <BIButton type="primary2" style={{ width: '70px' }} onClick={this.reset}>
              重置
            </BIButton>
          </div>
        </div>
