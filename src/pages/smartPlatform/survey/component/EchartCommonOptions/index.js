...
export function chartOptions(data) {
  ...
  const params1 = {
    text: '考试通知触达趋势',
    legendData: [
      { name: '考试计划人数', icon: 'circle' },
      { name: '通知人数', icon: 'circle' },
      { name: '触达人数', icon: 'circle' },
      {
        name: '触达率',
      },
    ],
    ...
    formatter: function(params) {
      return `<div style="min-width:200px;box-shadow:0 0 8px 0; border-radius: 5px;padding:12px 16px 8px ">${_html1(
        params.dataIndex,
        params.name
      )}</div>`;
    },
  ...
  const params2 = {
    ...
    formatter: function(params) {
      return `<div style="min-width:200px; border-radius: 5px;box-shadow:0 0 8px 0;padding:12px 16px 8px ">${_html2(
        params.dataIndex,
        params.name
      )}</div>`;
    },
